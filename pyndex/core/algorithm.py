...
@dec.validatebday(rank_day = True)
def get_quarter_rank(quarter_rebalance):
    preceeding_weeks = 5
...
def get_calendar(year):
    ...
    cal = {}
    cal["Annual Rank Day"] = get_annual_rank(year)
    cal["Annual Rebalance Day"] = get_rebalance(year,2)
    cal["Q3 Rebalance Day"] = get_rebalance(year,3)
    cal["Q4 Rebalance Day"] = get_rebalance(year,4)
    cal["Q1 Rebalance Day"] = get_rebalance(year,1)
    cal["Q3 Rank Day"] = get_quarter_rank(cal["Q3 Rebalance Day"])
    cal["Q4 Rank Day"] = get_quarter_rank(cal["Q4 Rebalance Day"])
    cal["Q1 Rank Day"] = get_quarter_rank(cal["Q1 Rebalance Day"])

    return cal

def match_rank_w_rebalance(cal,year):
    match = {}
...
def validate_year(wrds, year):
    last_date = wrds.raw_sql('''SELECT date FROM crsp_a_stock.dsf 
    ORDER BY date DESC LIMIT 1''')["date"].get(0)
    valid_year = (year >= 1989) & (get_quarter_rank(get_rebalance(year,1)) <= last_date)
    return valid_year

def download(wrds, year):
    if not validate_year(wrds, year):
    ...
    AND '{rank_y_plus_1}'
    ORDER BY date
    """.format(rank_y = get_annual_rank(year),
        rank_y_plus_1 =  get_annual_rank(year+1))
...
#-------------------Index-----------------------
def filter_by_shrcd(stock, metadata):
    common_stocks = metadata[(metadata.hshrcd == 10.0) 
                    | (metadata.hshrcd == 11.0)].permno
    return stock[stock.permno.isin(common_stocks)]        

def filter_by_price(stock):
    return stock[(stock["prc"].abs()/stock["cfacpr"] > 1.0)]

def filter_by_cap(stock):
    return stock[stock["mkt_cap"]>30e6]
    
def filter_by_minimum_requirements(stock):
    return filter_by_cap(filter_by_price(stock))

def filter_by_ranking(stock, index):
    if index == "3000":
...
def get_mkt_cap_by_permco(stock):
    return stock.groupby("permco",
                    as_index=False)["mkt_cap"].sum()
    
def append_weights(stock):
    mkt_cap_by_permco = get_mkt_cap_by_permco(stock)
...
def get_quarter_dates(quarter,calendar):
    if quarter == 3:
...
def get_new_IPOs(metadata, calendar, init= None, end = None):
    return metadata[(metadata.begdat > init) & (metadata.begdat <= end)].permno

def preprocessing_IPOs(metadata, stock, calendar, init = None, end = None):

    IPOs_ids = get_new_IPOs(metadata, calendar, init = init,
                 end = end)
    IPOs_stock = stock[(stock.permno.isin(IPOs_ids)) & 
                (stock.date == end)]
    filtered_ipos = filter_by_minimum_requirements(IPOs_stock)
    return filtered_ipos

def filter_by_capitalization(filtered_ipos, index_stocks, index):
    annual_mkt_cap_by_permco = get_mkt_cap_by_permco(index_stocks[
...
def filter_by_companies_in_index(filtered_ipos, index_stocks):
...
def build_annual_weights(calendar, stock, index, metadata):
    filtered_stocks = filter_by_minimum_requirements(
                    stock[stock.date == calendar["Annual Rank Day"]])
    filtered_stocks = filter_by_shrcd(filtered_stocks, metadata)
    filtered_stocks = filter_by_ranking(filtered_stocks, index)
    return append_weights(filtered_stocks)

def build_quarterly_weights(index_stocks, metadata, stock, 
                            calendar, index, quarter):
    init, end = get_quarter_dates(quarter,calendar)
    filtered_ipos = preprocessing_IPOs(metadata, stock, calendar,
                    init = init, end = end)
    filtered_ipos = filter_by_shrcd(filtered_ipos, metadata)           
    filtered_ipos = filter_by_capitalization(filtered_ipos,
                    index_stocks, index)
    filtered_ipos = filter_by_companies_in_index(filtered_ipos,
                    index_stocks)
    ...
    return  pd.concat([index_stocks,append_weights(new_index_composition)])

#----------------- Formatting -----------------------
def beautify_table(table, calendar, year):   
    table.date =  table.date.replace(
                    to_replace = match_rank_w_rebalance(calendar,year))
    weights_table = pd.pivot_table(table,  values='weights', index=['date'],
                 columns=['permno','cusip','permco'])
    return weights_table

def reindex_with_bday(weights_table,trading_days, calendar, year):
    last_bday = get_calendar(year+1)["Annual Rebalance Day"] - BDay(1)
    trading_days = pd.to_datetime(trading_days)
    index_from_rebalance = trading_days[trading_days.date >=calendar["Annual Rebalance Day"]]
...
def build(stock, year, index, metadata):
    calendar = get_calendar(year)
    stock = stock.assign(mkt_cap=stock.prc.abs()*stock.shrout*1000)
    index_stocks = build_annual_weights(calendar,
                    stock,index,metadata)
    if year > 2003:
        for quarter in [3,4,1]:
            index_stocks =  build_quarterly_weights(
                                    index_stocks, metadata, stock, calendar,
                                    index, quarter)
    weights_table = beautify_table(index_stocks, calendar, year)    
    return reindex_with_bday(weights_table,stock.date.unique(),calendar, year)
